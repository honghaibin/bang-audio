(function($,fn,obj){
            ...
            if(Object.prototype.toString.call(options) === '[object Array]'){
                ...
                for(var i=0;i<options.length;i++){
                    ...
                    if(option.download=== false){
                        container.find('.download:last').remove();
                    }
                    if(option.rate=== false){
                        container.find('.rate:last').remove();
                    }
                }
            }
                ...
                if(options.download=== false){
                    container.find('.download').remove();
                }
                if(options.rate=== false){
                    container.find('.rate').remove();
                }
            }
    	...
    	app.pt.openVolumeControls= function(e){
    		...
    		var volumeBtn= src.find('.volume-btn')[0];
    		var volumeControls= src.find('.volume-controls')[0];
    	...
    	app.pt.showTotalTime= function (e) {
    		...
    		var totalTime= src.find('.total-time');
    		var player= src.find('audio')[0];
    	...
    	app.pt.updateVolume= function(e){
    		...
    		var volumeControls= src.find('.volume-controls')
    		var volumeProgress= volumeControls.find('.slider .progress')[0];
    		var player= src.find('audio')[0];
    	...
    	app.pt.makePlay= function(e){
    		...
    		var playpauseBtn= src.find('.play-pause-btn');
    		var loading= src.find('.loading');
    		playpauseBtn.show();
  			loading.hide();
    	}
    	...
    	app.pt.updateProgress= function(e){
    		...
    		var player= src.find('audio')[0];
    		var progress= src.find('.progress')[0];
    		var currentTime= src.find('.current-time');
        ...
        app.pt.stopOtherAudio= function(e){
            $('.audio.bang-audio-player').each(function(){
                ...
                if(!player.paused && player!= e.target){
                    $(this).find('.play-pause-btn').trigger('click');
                }
            })
        }
	...
	{temp:'<div class="audio bang-audio-player">'
		...
		+'</div>'
		+'<div class="play-pause-btn" onclick="BANG_audio.togglePlay(event)">  '
		    +'<svg xmlns="http://www.w3.org/2000/svg" width="18" height="24" viewBox="0 0 18 24">'
		     	+'<path fill="#566574" fill-rule="evenodd" d="M18 12L0 24V0" class="play-pause-icon" data-role="playPause">'
		    +'</svg>'
		+'</div>'
		+'<div class="controls">'
    		+'<span class="current-time">0:00</span>'
    		+'<div class="slider" data-direction="horizontal" onclick="BANG_audio.rewind(event)">'
      			+'<div class="progress">'
  		...
  		+'</div>'
  		+'<div class="volume">'
		    +'<div class="volume-btn" onclick="BANG_audio.openVolumeControls(event)">'
		    ...
		    +'<div class="volume-controls hidden">'
		      	+'<div class="slider" data-direction="vertical" onclick="BANG_audio.changeVolume(event)">'
		        	+'<div class="progress">'
        ...
        +'</div>'
        +'<div class="rate">'
            +'<div class="rate-btn" onclick="BANG_audio.changeRate(event)">x1.0</div>'
        +'</div>'
		+'<audio '
		+'crossorigin '
